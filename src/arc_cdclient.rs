...
// Vec: When I don't know the grouping or there is none
pub type RefVec<T> = Vec<Arc<T>>;
pub fn vec_to_vec_ref<T>(v: &RefVec<T>) -> RefVec<T> {
    v.clone()
}

pub type RefMap<T> = BTreeMap<<T as HasKey>::Key, Arc<T>>;
...
pub fn vec_to_map<T: HasKey>(v: &RefVec<T>) -> RefMap<T> {
    v.iter().map(|it| (it.clone_key(), it.clone())).collect()
}
...
* at that key
*/
pub type RefGroupedMap<T> = BTreeMap<<T as HasGroupKey>::Key, RefVec<T>>;
...
pub fn vec_to_grouped_map<T: HasGroupKey>(v: &RefVec<T>) -> RefGroupedMap<T> {
    let mut grouped_map: RefGroupedMap<T> = BTreeMap::new();
    for it in v.iter() {
        if let Some(entry) = grouped_map.get_mut(it.get_group_key()) {
            entry.push(it.clone());
        } else {
            grouped_map.insert(it.clone_key(), vec![it.clone()]);
        }
    }
    grouped_map
}
...
pub struct CdClientRows {
    pub ai_combat_roles: RefVec<AiCombatRoles>,
    pub accessory_default_loc: RefVec<AccessoryDefaultLoc>,
    pub activities: RefVec<Activities>,
    pub activity_rewards: RefVec<ActivityRewards>,
    pub activity_text: RefVec<ActivityText>,
    pub animation_index: RefVec<AnimationIndex>,
    pub animations: RefVec<Animations>,
    pub base_combat_aicomponent: RefVec<BaseCombatAIComponent>,
    pub behavior_effect: RefVec<BehaviorEffect>,
    pub behavior_parameter: RefVec<BehaviorParameter>,
    pub behavior_template: RefVec<BehaviorTemplate>,
    pub behavior_template_name: RefVec<BehaviorTemplateName>,
    pub blueprints: RefVec<Blueprints>,
    pub brick_colors: RefVec<BrickColors>,
    pub brick_idtable: RefVec<BrickIDTable>,
    pub buff_definitions: RefVec<BuffDefinitions>,
    pub buff_parameters: RefVec<BuffParameters>,
    pub camera: RefVec<Camera>,
    pub celebration_parameters: RefVec<CelebrationParameters>,
    pub choice_build_component: RefVec<ChoiceBuildComponent>,
    pub collectible_component: RefVec<CollectibleComponent>,
    pub components_registry: RefVec<ComponentsRegistry>,
    pub control_schemes: RefVec<ControlSchemes>,
    pub currency_denominations: RefVec<CurrencyDenominations>,
    pub currency_table: RefVec<CurrencyTable>,
    pub db_exclude: RefVec<DbExclude>,
    pub deletion_restrictions: RefVec<DeletionRestrictions>,
    pub destructible_component: RefVec<DestructibleComponent>,
    pub dev_model_behaviors: RefVec<DevModelBehaviors>,
    pub emotes: RefVec<Emotes>,
    pub event_gating: RefVec<EventGating>,
    pub exhibit_component: RefVec<ExhibitComponent>,
    pub factions: RefVec<Factions>,
    pub feature_gating: RefVec<FeatureGating>,
    pub flair_table: RefVec<FlairTable>,
    pub icons: RefVec<Icons>,
    pub inventory_component: RefVec<InventoryComponent>,
    pub item_component: RefVec<ItemComponent>,
    pub item_egg_data: RefVec<ItemEggData>,
    pub item_food_data: RefVec<ItemFoodData>,
    pub item_set_skills: RefVec<ItemSetSkills>,
    pub item_sets: RefVec<ItemSets>,
    pub jet_pack_pad_component: RefVec<JetPackPadComponent>,
    pub lu_pexhibit_component: RefVec<LuPExhibitComponent>,
    pub lu_pexhibit_model_data: RefVec<LuPExhibitModelData>,
    pub lu_pzone_ids: RefVec<LuPZoneIDs>,
    pub language_type: RefVec<LanguageType>,
    pub level_progression_lookup: RefVec<LevelProgressionLookup>,
    pub loot_matrix: RefVec<LootMatrix>,
    pub loot_matrix_index: RefVec<LootMatrixIndex>,
    pub loot_table: RefVec<LootTable>,
    pub loot_table_index: RefVec<LootTableIndex>,
    pub minifig_component: RefVec<MinifigComponent>,
    pub minifig_decals_eyebrows: RefVec<MinifigDecalsEyebrows>,
    pub minifig_decals_eyes: RefVec<MinifigDecalsEyes>,
    pub minifig_decals_legs: RefVec<MinifigDecalsLegs>,
    pub minifig_decals_mouths: RefVec<MinifigDecalsMouths>,
    pub minifig_decals_torsos: RefVec<MinifigDecalsTorsos>,
    pub mission_email: RefVec<MissionEmail>,
    pub mission_npccomponent: RefVec<MissionNPCComponent>,
    pub mission_tasks: RefVec<MissionTasks>,
    pub mission_text: RefVec<MissionText>,
    pub missions: RefVec<Missions>,
    pub model_behavior: RefVec<ModelBehavior>,
    pub modular_build_component: RefVec<ModularBuildComponent>,
    pub module_component: RefVec<ModuleComponent>,
    pub motion_fx: RefVec<MotionFX>,
    pub movement_aicomponent: RefVec<MovementAIComponent>,
    pub moving_platforms: RefVec<MovingPlatforms>,
    pub npc_icons: RefVec<NpcIcons>,
    pub object_behavior_xref: RefVec<ObjectBehaviorXREF>,
    pub object_behaviors: RefVec<ObjectBehaviors>,
    pub object_skills: RefVec<ObjectSkills>,
    pub objects: RefVec<Objects>,
    pub package_component: RefVec<PackageComponent>,
    pub pet_abilities: RefVec<PetAbilities>,
    pub pet_component: RefVec<PetComponent>,
    pub pet_nest_component: RefVec<PetNestComponent>,
    pub physics_component: RefVec<PhysicsComponent>,
    pub player_flags: RefVec<PlayerFlags>,
    pub player_statistics: RefVec<PlayerStatistics>,
    pub preconditions: RefVec<Preconditions>,
    pub property_entrance_component: RefVec<PropertyEntranceComponent>,
    pub property_template: RefVec<PropertyTemplate>,
    pub proximity_monitor_component: RefVec<ProximityMonitorComponent>,
    pub proximity_types: RefVec<ProximityTypes>,
    pub racing_module_component: RefVec<RacingModuleComponent>,
    pub rail_activator_component: RefVec<RailActivatorComponent>,
    pub rarity_table: RefVec<RarityTable>,
    pub rarity_table_index: RefVec<RarityTableIndex>,
    pub rebuild_component: RefVec<RebuildComponent>,
    pub rebuild_sections: RefVec<RebuildSections>,
    pub release_version: RefVec<ReleaseVersion>,
    pub render_component: RefVec<RenderComponent>,
    pub render_component_flash: RefVec<RenderComponentFlash>,
    pub render_component_wrapper: RefVec<RenderComponentWrapper>,
    pub render_icon_assets: RefVec<RenderIconAssets>,
    pub reputation_rewards: RefVec<ReputationRewards>,
    pub reward_codes: RefVec<RewardCodes>,
    pub rewards: RefVec<Rewards>,
    pub rocket_launchpad_control_component: RefVec<RocketLaunchpadControlComponent>,
    pub scene_table: RefVec<SceneTable>,
    pub script_component: RefVec<ScriptComponent>,
    pub skill_behavior: RefVec<SkillBehavior>,
    pub smashable_chain: RefVec<SmashableChain>,
    pub smashable_chain_index: RefVec<SmashableChainIndex>,
    pub smashable_component: RefVec<SmashableComponent>,
    pub smashable_elements: RefVec<SmashableElements>,
    pub speedchat_menu: RefVec<SpeedchatMenu>,
    pub subscription_pricing: RefVec<SubscriptionPricing>,
    pub surface_type: RefVec<SurfaceType>,
    pub taming_build_puzzles: RefVec<TamingBuildPuzzles>,
    pub text_description: RefVec<TextDescription>,
    pub text_language: RefVec<TextLanguage>,
    pub trail_effects: RefVec<TrailEffects>,
    pub ug_behavior_sounds: RefVec<UgBehaviorSounds>,
    pub vehicle_physics: RefVec<VehiclePhysics>,
    pub vehicle_stat_map: RefVec<VehicleStatMap>,
    pub vendor_component: RefVec<VendorComponent>,
    pub whats_cool_item_spotlight: RefVec<WhatsCoolItemSpotlight>,
    pub whats_cool_news_and_tips: RefVec<WhatsCoolNewsAndTips>,
    pub world_config: RefVec<WorldConfig>,
    pub zone_loading_tips: RefVec<ZoneLoadingTips>,
    pub zone_summary: RefVec<ZoneSummary>,
    pub zone_table: RefVec<ZoneTable>,
    pub brick_attributes: RefVec<BrickAttributes>,
    pub dtproperties: RefVec<Dtproperties>,
    pub map_animation_priorities: RefVec<MapAnimationPriorities>,
    pub map_asset_type: RefVec<MapAssetType>,
    pub map_icon: RefVec<MapIcon>,
    pub map_item_types: RefVec<MapItemTypes>,
    pub map_render_effects: RefVec<MapRenderEffects>,
    pub map_shaders: RefVec<MapShaders>,
    pub map_texture_resource: RefVec<MapTextureResource>,
    pub map_blueprint_category: RefVec<MapBlueprintCategory>,
    pub sysdiagrams: RefVec<Sysdiagrams>,
    pub possessable_component: RefVec<PossessableComponent>,
    pub skill_sets: RefVec<SkillSets>,
    pub map_faces_and_hair: RefVec<MapFacesAndHair>,
}
...
#[allow(dead_code)]
pub struct CdClient {
    pub rows: CdClientRows,
    pub ai_combat_roles: RefMap<AiCombatRoles>,
    pub accessory_default_loc: RefMap<AccessoryDefaultLoc>,
    pub activities: RefMap<Activities>,
    pub activity_rewards: RefGroupedMap<ActivityRewards>,
    pub activity_text: RefGroupedMap<ActivityText>,
    pub animation_index: RefMap<AnimationIndex>,
    pub animations: RefGroupedMap<Animations>,
    pub base_combat_aicomponent: RefMap<BaseCombatAIComponent>,
    pub behavior_effect: RefGroupedMap<BehaviorEffect>,
    pub behavior_parameter: RefGroupedMap<BehaviorParameter>,
    pub behavior_template: RefGroupedMap<BehaviorTemplate>,
    pub behavior_template_name: RefMap<BehaviorTemplateName>,
    pub blueprints: RefMap<Blueprints>,
    pub brick_colors: RefMap<BrickColors>,
    pub brick_idtable: RefMap<BrickIDTable>,
    pub buff_definitions: RefMap<BuffDefinitions>,
    pub buff_parameters: RefMap<BuffParameters>,
    pub camera: RefMap<Camera>,
    pub celebration_parameters: RefMap<CelebrationParameters>,
    pub choice_build_component: RefMap<ChoiceBuildComponent>,
    pub collectible_component: RefMap<CollectibleComponent>,
    pub components_registry: RefGroupedMap<ComponentsRegistry>,
    pub control_schemes: RefMap<ControlSchemes>,
    pub currency_denominations: RefMap<CurrencyDenominations>,
    pub currency_table: RefMap<CurrencyTable>,
    pub db_exclude: RefMap<DbExclude>,
    pub deletion_restrictions: RefMap<DeletionRestrictions>,
    pub destructible_component: RefMap<DestructibleComponent>,
    pub dev_model_behaviors: RefMap<DevModelBehaviors>,
    pub emotes: RefMap<Emotes>,
    pub event_gating: RefMap<EventGating>,
    pub exhibit_component: RefMap<ExhibitComponent>,
    pub factions: RefMap<Factions>,
    pub feature_gating: RefMap<FeatureGating>,
    pub flair_table: RefMap<FlairTable>,
    pub icons: RefMap<Icons>,
    pub inventory_component: RefVec<InventoryComponent>,
    pub item_component: RefMap<ItemComponent>,
    pub item_egg_data: RefMap<ItemEggData>,
    pub item_food_data: RefMap<ItemFoodData>,
    pub item_set_skills: RefGroupedMap<ItemSetSkills>,
    pub item_sets: RefMap<ItemSets>,
    pub jet_pack_pad_component: RefMap<JetPackPadComponent>,
    pub lu_pexhibit_component: RefMap<LuPExhibitComponent>,
    pub lu_pexhibit_model_data: RefMap<LuPExhibitModelData>,
    pub lu_pzone_ids: RefMap<LuPZoneIDs>,
    pub language_type: RefMap<LanguageType>,
    pub level_progression_lookup: RefMap<LevelProgressionLookup>,
    pub loot_matrix: RefGroupedMap<LootMatrix>,
    pub loot_matrix_index: RefMap<LootMatrixIndex>,
    pub loot_table: RefVec<LootTable>,
    pub loot_table_index: RefMap<LootTableIndex>,
    pub minifig_component: RefMap<MinifigComponent>,
    pub minifig_decals_eyebrows: RefMap<MinifigDecalsEyebrows>,
    pub minifig_decals_eyes: RefMap<MinifigDecalsEyes>,
    pub minifig_decals_legs: RefMap<MinifigDecalsLegs>,
    pub minifig_decals_mouths: RefMap<MinifigDecalsMouths>,
    pub minifig_decals_torsos: RefMap<MinifigDecalsTorsos>,
    pub mission_email: RefMap<MissionEmail>,
    pub mission_npccomponent: RefVec<MissionNPCComponent>,
    pub mission_tasks: RefVec<MissionTasks>,
    pub mission_text: RefMap<MissionText>,
    pub missions: RefMap<Missions>,
    pub model_behavior: RefMap<ModelBehavior>,
    pub modular_build_component: RefMap<ModularBuildComponent>,
    pub module_component: RefMap<ModuleComponent>,
    pub motion_fx: RefMap<MotionFX>,
    pub movement_aicomponent: RefMap<MovementAIComponent>,
    pub moving_platforms: RefMap<MovingPlatforms>,
    pub npc_icons: RefMap<NpcIcons>,
    pub object_behavior_xref: RefMap<ObjectBehaviorXREF>,
    pub object_behaviors: RefMap<ObjectBehaviors>,
    pub object_skills: RefGroupedMap<ObjectSkills>,
    pub objects: RefMap<Objects>,
    pub package_component: RefMap<PackageComponent>,
    pub pet_abilities: RefMap<PetAbilities>,
    pub pet_component: RefMap<PetComponent>,
    pub pet_nest_component: RefMap<PetNestComponent>,
    pub physics_component: RefMap<PhysicsComponent>,
    pub player_flags: RefMap<PlayerFlags>,
    pub player_statistics: RefMap<PlayerStatistics>,
    pub preconditions: RefMap<Preconditions>,
    pub property_entrance_component: RefMap<PropertyEntranceComponent>,
    pub property_template: RefMap<PropertyTemplate>,
    pub proximity_monitor_component: RefMap<ProximityMonitorComponent>,
    pub proximity_types: RefMap<ProximityTypes>,
    pub racing_module_component: RefMap<RacingModuleComponent>,
    pub rail_activator_component: RefMap<RailActivatorComponent>,
    pub rarity_table: RefMap<RarityTable>,
    pub rarity_table_index: RefMap<RarityTableIndex>,
    pub rebuild_component: RefMap<RebuildComponent>,
    pub rebuild_sections: RefMap<RebuildSections>,
    pub release_version: RefMap<ReleaseVersion>,
    pub render_component: RefMap<RenderComponent>,
    pub render_component_flash: RefGroupedMap<RenderComponentFlash>,
    pub render_component_wrapper: RefMap<RenderComponentWrapper>,
    pub render_icon_assets: RefMap<RenderIconAssets>,
    pub reputation_rewards: RefMap<ReputationRewards>,
    pub reward_codes: RefMap<RewardCodes>,
    pub rewards: RefMap<Rewards>,
    pub rocket_launchpad_control_component: RefMap<RocketLaunchpadControlComponent>,
    pub scene_table: RefMap<SceneTable>,
    pub script_component: RefMap<ScriptComponent>,
    pub skill_behavior: RefMap<SkillBehavior>,
    pub smashable_chain: RefGroupedMap<SmashableChain>,
    pub smashable_chain_index: RefMap<SmashableChainIndex>,
    pub smashable_component: RefMap<SmashableComponent>,
    pub smashable_elements: RefMap<SmashableElements>,
    pub speedchat_menu: RefMap<SpeedchatMenu>,
    pub subscription_pricing: RefMap<SubscriptionPricing>,
    pub surface_type: RefMap<SurfaceType>,
    pub taming_build_puzzles: RefMap<TamingBuildPuzzles>,
    pub text_description: RefMap<TextDescription>,
    pub text_language: RefMap<TextLanguage>,
    pub trail_effects: RefMap<TrailEffects>,
    pub ug_behavior_sounds: RefMap<UgBehaviorSounds>,
    pub vehicle_physics: RefMap<VehiclePhysics>,
    pub vehicle_stat_map: RefGroupedMap<VehicleStatMap>,
    pub vendor_component: RefMap<VendorComponent>,
    pub whats_cool_item_spotlight: RefMap<WhatsCoolItemSpotlight>,
    pub whats_cool_news_and_tips: RefMap<WhatsCoolNewsAndTips>,
    pub world_config: RefMap<WorldConfig>,
    pub zone_loading_tips: RefMap<ZoneLoadingTips>,
    pub zone_summary: RefGroupedMap<ZoneSummary>,
    pub zone_table: RefMap<ZoneTable>,
    pub brick_attributes: RefMap<BrickAttributes>,
    pub dtproperties: RefMap<Dtproperties>,
    pub map_animation_priorities: RefMap<MapAnimationPriorities>,
    pub map_asset_type: RefMap<MapAssetType>,
    pub map_icon: RefGroupedMap<MapIcon>,
    pub map_item_types: RefMap<MapItemTypes>,
    pub map_render_effects: RefMap<MapRenderEffects>,
    pub map_shaders: RefMap<MapShaders>,
    pub map_texture_resource: RefMap<MapTextureResource>,
    pub map_blueprint_category: RefMap<MapBlueprintCategory>,
    pub sysdiagrams: RefMap<Sysdiagrams>,
    pub possessable_component: RefMap<PossessableComponent>,
    pub skill_sets: RefMap<SkillSets>,
    pub map_faces_and_hair: RefMap<MapFacesAndHair>,
}
...
impl CdClient {
    pub fn load_sqlite(path: &Path) -> rusqlite::Result<Self> {
        ...
        Ok(Self {
            ai_combat_roles: vec_to_map(&cdclient.ai_combat_roles),
            accessory_default_loc: vec_to_map(&cdclient.accessory_default_loc),
            activities: vec_to_map(&cdclient.activities),
            activity_rewards: vec_to_grouped_map(&cdclient.activity_rewards),
            activity_text: vec_to_grouped_map(&cdclient.activity_text),
            animation_index: vec_to_map(&cdclient.animation_index),
            animations: vec_to_grouped_map(&cdclient.animations),
            base_combat_aicomponent: vec_to_map(&cdclient.base_combat_aicomponent),
            behavior_effect: vec_to_grouped_map(&cdclient.behavior_effect),
            behavior_parameter: vec_to_grouped_map(&cdclient.behavior_parameter),
            behavior_template: vec_to_grouped_map(&cdclient.behavior_template),
            behavior_template_name: vec_to_map(&cdclient.behavior_template_name),
            blueprints: vec_to_map(&cdclient.blueprints),
            brick_colors: vec_to_map(&cdclient.brick_colors),
            brick_idtable: vec_to_map(&cdclient.brick_idtable),
            buff_definitions: vec_to_map(&cdclient.buff_definitions),
            buff_parameters: vec_to_map(&cdclient.buff_parameters),
            camera: vec_to_map(&cdclient.camera),
            celebration_parameters: vec_to_map(&cdclient.celebration_parameters),
            choice_build_component: vec_to_map(&cdclient.choice_build_component),
            collectible_component: vec_to_map(&cdclient.collectible_component),
            components_registry: vec_to_grouped_map(&cdclient.components_registry),
            control_schemes: vec_to_map(&cdclient.control_schemes),
            currency_denominations: vec_to_map(&cdclient.currency_denominations),
            currency_table: vec_to_map(&cdclient.currency_table),
            db_exclude: vec_to_map(&cdclient.db_exclude),
            deletion_restrictions: vec_to_map(&cdclient.deletion_restrictions),
            destructible_component: vec_to_map(&cdclient.destructible_component),
            dev_model_behaviors: vec_to_map(&cdclient.dev_model_behaviors),
            emotes: vec_to_map(&cdclient.emotes),
            event_gating: vec_to_map(&cdclient.event_gating),
            exhibit_component: vec_to_map(&cdclient.exhibit_component),
            factions: vec_to_map(&cdclient.factions),
            feature_gating: vec_to_map(&cdclient.feature_gating),
            flair_table: vec_to_map(&cdclient.flair_table),
            icons: vec_to_map(&cdclient.icons),
            inventory_component: vec_to_vec_ref(&cdclient.inventory_component),
            item_component: vec_to_map(&cdclient.item_component),
            item_egg_data: vec_to_map(&cdclient.item_egg_data),
            item_food_data: vec_to_map(&cdclient.item_food_data),
            item_set_skills: vec_to_grouped_map(&cdclient.item_set_skills),
            item_sets: vec_to_map(&cdclient.item_sets),
            jet_pack_pad_component: vec_to_map(&cdclient.jet_pack_pad_component),
            lu_pexhibit_component: vec_to_map(&cdclient.lu_pexhibit_component),
            lu_pexhibit_model_data: vec_to_map(&cdclient.lu_pexhibit_model_data),
            lu_pzone_ids: vec_to_map(&cdclient.lu_pzone_ids),
            language_type: vec_to_map(&cdclient.language_type),
            level_progression_lookup: vec_to_map(&cdclient.level_progression_lookup),
            loot_matrix: vec_to_grouped_map(&cdclient.loot_matrix),
            loot_matrix_index: vec_to_map(&cdclient.loot_matrix_index),
            loot_table: vec_to_vec_ref(&cdclient.loot_table),
            loot_table_index: vec_to_map(&cdclient.loot_table_index),
            minifig_component: vec_to_map(&cdclient.minifig_component),
            minifig_decals_eyebrows: vec_to_map(&cdclient.minifig_decals_eyebrows),
            minifig_decals_eyes: vec_to_map(&cdclient.minifig_decals_eyes),
            minifig_decals_legs: vec_to_map(&cdclient.minifig_decals_legs),
            minifig_decals_mouths: vec_to_map(&cdclient.minifig_decals_mouths),
            minifig_decals_torsos: vec_to_map(&cdclient.minifig_decals_torsos),
            mission_email: vec_to_map(&cdclient.mission_email),
            mission_npccomponent: vec_to_vec_ref(&cdclient.mission_npccomponent),
            mission_tasks: vec_to_vec_ref(&cdclient.mission_tasks),
            mission_text: vec_to_map(&cdclient.mission_text),
            missions: vec_to_map(&cdclient.missions),
            model_behavior: vec_to_map(&cdclient.model_behavior),
            modular_build_component: vec_to_map(&cdclient.modular_build_component),
            module_component: vec_to_map(&cdclient.module_component),
            motion_fx: vec_to_map(&cdclient.motion_fx),
            movement_aicomponent: vec_to_map(&cdclient.movement_aicomponent),
            moving_platforms: vec_to_map(&cdclient.moving_platforms),
            npc_icons: vec_to_map(&cdclient.npc_icons),
            object_behavior_xref: vec_to_map(&cdclient.object_behavior_xref),
            object_behaviors: vec_to_map(&cdclient.object_behaviors),
            object_skills: vec_to_grouped_map(&cdclient.object_skills),
            objects: vec_to_map(&cdclient.objects),
            package_component: vec_to_map(&cdclient.package_component),
            pet_abilities: vec_to_map(&cdclient.pet_abilities),
            pet_component: vec_to_map(&cdclient.pet_component),
            pet_nest_component: vec_to_map(&cdclient.pet_nest_component),
            physics_component: vec_to_map(&cdclient.physics_component),
            player_flags: vec_to_map(&cdclient.player_flags),
            player_statistics: vec_to_map(&cdclient.player_statistics),
            preconditions: vec_to_map(&cdclient.preconditions),
            property_entrance_component: vec_to_map(&cdclient.property_entrance_component),
            property_template: vec_to_map(&cdclient.property_template),
            proximity_monitor_component: vec_to_map(&cdclient.proximity_monitor_component),
            proximity_types: vec_to_map(&cdclient.proximity_types),
            racing_module_component: vec_to_map(&cdclient.racing_module_component),
            rail_activator_component: vec_to_map(&cdclient.rail_activator_component),
            rarity_table: vec_to_map(&cdclient.rarity_table),
            rarity_table_index: vec_to_map(&cdclient.rarity_table_index),
            rebuild_component: vec_to_map(&cdclient.rebuild_component),
            rebuild_sections: vec_to_map(&cdclient.rebuild_sections),
            release_version: vec_to_map(&cdclient.release_version),
            render_component: vec_to_map(&cdclient.render_component),
            render_component_flash: vec_to_grouped_map(&cdclient.render_component_flash),
            render_component_wrapper: vec_to_map(&cdclient.render_component_wrapper),
            render_icon_assets: vec_to_map(&cdclient.render_icon_assets),
            reputation_rewards: vec_to_map(&cdclient.reputation_rewards),
            reward_codes: vec_to_map(&cdclient.reward_codes),
            rewards: vec_to_map(&cdclient.rewards),
            rocket_launchpad_control_component: vec_to_map(
                &cdclient.rocket_launchpad_control_component,
            ),
            scene_table: vec_to_map(&cdclient.scene_table),
            script_component: vec_to_map(&cdclient.script_component),
            skill_behavior: vec_to_map(&cdclient.skill_behavior),
            smashable_chain: vec_to_grouped_map(&cdclient.smashable_chain),
            smashable_chain_index: vec_to_map(&cdclient.smashable_chain_index),
            smashable_component: vec_to_map(&cdclient.smashable_component),
            smashable_elements: vec_to_map(&cdclient.smashable_elements),
            speedchat_menu: vec_to_map(&cdclient.speedchat_menu),
            subscription_pricing: vec_to_map(&cdclient.subscription_pricing),
            surface_type: vec_to_map(&cdclient.surface_type),
            taming_build_puzzles: vec_to_map(&cdclient.taming_build_puzzles),
            text_description: vec_to_map(&cdclient.text_description),
            text_language: vec_to_map(&cdclient.text_language),
            trail_effects: vec_to_map(&cdclient.trail_effects),
            ug_behavior_sounds: vec_to_map(&cdclient.ug_behavior_sounds),
            vehicle_physics: vec_to_map(&cdclient.vehicle_physics),
            vehicle_stat_map: vec_to_grouped_map(&cdclient.vehicle_stat_map),
            vendor_component: vec_to_map(&cdclient.vendor_component),
            whats_cool_item_spotlight: vec_to_map(&cdclient.whats_cool_item_spotlight),
            whats_cool_news_and_tips: vec_to_map(&cdclient.whats_cool_news_and_tips),
            world_config: vec_to_map(&cdclient.world_config),
            zone_loading_tips: vec_to_map(&cdclient.zone_loading_tips),
            zone_summary: vec_to_grouped_map(&cdclient.zone_summary),
            zone_table: vec_to_map(&cdclient.zone_table),
            brick_attributes: vec_to_map(&cdclient.brick_attributes),
            dtproperties: vec_to_map(&cdclient.dtproperties),
            map_animation_priorities: vec_to_map(&cdclient.map_animation_priorities),
            map_asset_type: vec_to_map(&cdclient.map_asset_type),
            map_icon: vec_to_grouped_map(&cdclient.map_icon),
            map_item_types: vec_to_map(&cdclient.map_item_types),
            map_render_effects: vec_to_map(&cdclient.map_render_effects),
            map_shaders: vec_to_map(&cdclient.map_shaders),
            map_texture_resource: vec_to_map(&cdclient.map_texture_resource),
            map_blueprint_category: vec_to_map(&cdclient.map_blueprint_category),
            sysdiagrams: vec_to_map(&cdclient.sysdiagrams),
            possessable_component: vec_to_map(&cdclient.possessable_component),
            skill_sets: vec_to_map(&cdclient.skill_sets),
            map_faces_and_hair: vec_to_map(&cdclient.map_faces_and_hair),
            rows: cdclient,
        })
    }
}
...
impl FromCdClient for Animations {
    ...
    fn query_map(row: &rusqlite::Row<'_>) -> rusqlite::Result<Self> {
        Ok(Self {
            ...
            face_animation_name: row.get(10)?,
            priority: row.get(11)?,
...
impl FromCdClient for CelebrationParameters {
    ...
    fn query_map(row: &rusqlite::Row<'_>) -> rusqlite::Result<Self> {
        Ok(Self {
            ...
            music_cue: row.get(27)?,
            sound_guid: row.get(28)?,
            mixer_program: row.get(29)?,
        })
    }
}
...
impl FromCdClient for Factions {
    ...
    fn query_map(row: &rusqlite::Row<'_>) -> rusqlite::Result<Self> {
        Ok(Self {
            ...
            friend_list: row.get(3)?,
            enemy_list: row.get(4)?,
        })
    }
}
...
impl FromCdClient for ItemComponent {
    ...
    fn query_map(row: &rusqlite::Row<'_>) -> rusqlite::Result<Self> {
        Ok(Self {
            ...
            sub_items: row.get(31)?,
            audio_event_use: row.get(32)?,
            no_equip_animation: row.get(33)?,
            commendation_lot: row.get(34)?,
            commendation_cost: row.get(35)?,
            audio_equip_meta_event_set: row.get(36)?,
            currency_costs: row.get(37)?,
            ingredient_info: row.get(38)?,
...
impl FromCdClient for MissionTasks {
    ...
    fn query_map(row: &rusqlite::Row<'_>) -> rusqlite::Result<Self> {
        Ok(Self {
            ...
            task_param1: row.get(6)?,
            large_task_icon: row.get(7)?,
            icon_id: row.get(8)?,
            uid: row.get(9)?,
...
impl FromCdClient for MissionText {
    ...
    fn query_map(row: &rusqlite::Row<'_>) -> rusqlite::Result<Self> {
        Ok(Self {
            id: row.get(0)?,
            story_icon: row.get(1)?,
            mission_icon: row.get(2)?,
            offer_npcicon: row.get(3)?,
            icon_id: row.get(4)?,
            state_1_anim: row.get(5)?,
            state_2_anim: row.get(6)?,
            state_3_anim: row.get(7)?,
            state_4_anim: row.get(8)?,
            state_3_turnin_anim: row.get(9)?,
            state_4_turnin_anim: row.get(10)?,
            onclick_anim: row.get(11)?,
            cinematic_accepted: row.get(12)?,
            cinematic_accepted_leadin: row.get(13)?,
            cinematic_completed: row.get(14)?,
            cinematic_completed_leadin: row.get(15)?,
            cinematic_repeatable: row.get(16)?,
            cinematic_repeatable_leadin: row.get(17)?,
            cinematic_repeatable_completed: row.get(18)?,
            cinematic_repeatable_completed_leadin: row.get(19)?,
            audio_event_guid_interact: row.get(20)?,
            audio_event_guid_offer_accept: row.get(21)?,
            audio_event_guid_offer_deny: row.get(22)?,
            audio_event_guid_completed: row.get(23)?,
            audio_event_guid_turn_in: row.get(24)?,
            audio_event_guid_failed: row.get(25)?,
            audio_event_guid_progress: row.get(26)?,
            audio_music_cue_offer_accept: row.get(27)?,
            audio_music_cue_turn_in: row.get(28)?,
            turn_in_icon_id: row.get(29)?,
            localize: row.get(30)?,
            loc_status: row.get(31)?,
            gate_version: row.get(32)?,
        })
    }
}
...
impl FromCdClient for Missions {
    ...
    fn query_map(row: &rusqlite::Row<'_>) -> rusqlite::Result<Self> {
        Ok(Self {
            ...
            prereq_mission_id: row.get(41)?,
            localize: row.get(42)?,
            in_motd: row.get(43)?,
            cooldown_time: row.get(44)?,
            is_random: row.get(45)?,
            random_pool: row.get(46)?,
            uiprereq_id: row.get(47)?,
            gate_version: row.get(48)?,
            hudstates: row.get(49)?,
...
impl FromCdClient for ModularBuildComponent {
    ...
    fn query_map(row: &rusqlite::Row<'_>) -> rusqlite::Result<Self> {
        Ok(Self {
            ...
            audio_event_guid_complete: row.get(6)?,
            audio_event_guid_present: row.get(7)?,
        })
    }
}
...
impl FromCdClient for PetComponent {
    ...
    fn query_map(row: &rusqlite::Row<'_>) -> rusqlite::Result<Self> {
        Ok(Self {
            ...
            audio_meta_event_set: row.get(13)?,
            buff_ids: row.get(14)?,
        })
    }
}
...
impl FromCdClient for PhysicsComponent {
    ...
    fn query_map(row: &rusqlite::Row<'_>) -> rusqlite::Result<Self> {
        Ok(Self {
            ...
            boundary_asset: row.get(12)?,
            jump_air_speed: row.get(13)?,
            friction: row.get(14)?,
            gravity_volume_asset: row.get(15)?,
        })
    }
}
...
impl FromCdClient for RailActivatorComponent {
    ...
    fn query_map(row: &rusqlite::Row<'_>) -> rusqlite::Result<Self> {
        Ok(Self {
            ...
            loop_anim: row.get(2)?,
            stop_anim: row.get(3)?,
            start_sound: row.get(4)?,
            loop_sound: row.get(5)?,
            stop_sound: row.get(6)?,
            effect_ids: row.get(7)?,
            preconditions: row.get(8)?,
            player_collision: row.get(9)?,
            camera_locked: row.get(10)?,
            start_effect_id: row.get(11)?,
            stop_effect_id: row.get(12)?,
...
impl FromCdClient for RenderComponent {
    ...
    fn query_map(row: &rusqlite::Row<'_>) -> rusqlite::Result<Self> {
        Ok(Self {
            ...
            effect5: row.get(9)?,
            effect6: row.get(10)?,
            animation_group_ids: row.get(11)?,
            fade: row.get(12)?,
...
impl FromCdClient for RocketLaunchpadControlComponent {
    ...
    fn query_map(row: &rusqlite::Row<'_>) -> rusqlite::Result<Self> {
        Ok(Self {
            ...
            target_scene: row.get(3)?,
            gm_level: row.get(4)?,
            player_animation: row.get(5)?,
            rocket_animation: row.get(6)?,
            launch_music: row.get(7)?,
            use_launch_precondition: row.get(8)?,
            use_alt_landing_precondition: row.get(9)?,
            launch_precondition: row.get(10)?,
            alt_landing_precondition: row.get(11)?,
            alt_landing_spawn_point_name: row.get(12)?,
        })
    }
}
...
impl FromCdClient for SurfaceType {
    ...
    fn query_map(row: &rusqlite::Row<'_>) -> rusqlite::Result<Self> {
        Ok(Self {
            ...
            footstep_ndaudio_meta_event_set_name: row.get(1)?,
        })
    }
}
...
impl FromCdClient for VehiclePhysics {
    ...
    fn query_map(row: &rusqlite::Row<'_>) -> rusqlite::Result<Self> {
        Ok(Self {
            ...
            audio_event_engine: row.get(76)?,
            audio_event_skid: row.get(77)?,
            audio_event_light_hit: row.get(78)?,
            audio_speed_threshold_light_hit: row.get(79)?,
            audio_timeout_light_hit: row.get(80)?,
            audio_event_heavy_hit: row.get(81)?,
            audio_speed_threshold_heavy_hit: row.get(82)?,
            audio_timeout_heavy_hit: row.get(83)?,
            audio_event_start: row.get(84)?,
            audio_event_tread_concrete: row.get(85)?,
            audio_event_tread_sand: row.get(86)?,
            audio_event_tread_wood: row.get(87)?,
            audio_event_tread_dirt: row.get(88)?,
            audio_event_tread_plastic: row.get(89)?,
            audio_event_tread_grass: row.get(90)?,
            audio_event_tread_gravel: row.get(91)?,
            audio_event_tread_mud: row.get(92)?,
            audio_event_tread_water: row.get(93)?,
            audio_event_tread_snow: row.get(94)?,
            audio_event_tread_ice: row.get(95)?,
            audio_event_tread_metal: row.get(96)?,
            audio_event_tread_leaves: row.get(97)?,
            audio_event_light_land: row.get(98)?,
            audio_airtime_for_light_land: row.get(99)?,
            audio_event_heavy_land: row.get(100)?,
...
impl FromCdClient for ZoneTable {
    ...
    fn query_map(row: &rusqlite::Row<'_>) -> rusqlite::Result<Self> {
        Ok(Self {
            ...
            display_description: row.get(8)?,
            map_folder: row.get(9)?,
            smashable_min_distance: row.get(10)?,
            smashable_max_distance: row.get(11)?,
            mixer_program: row.get(12)?,
            client_physics_framerate: row.get(13)?,
            server_physics_framerate: row.get(14)?,
...
impl FromCdClient for PossessableComponent {
    ...
    fn query_map(row: &rusqlite::Row<'_>) -> rusqlite::Result<Self> {
        Ok(Self {
            ...
            minifig_attach_point: row.get(2)?,
            minifig_attach_animation: row.get(3)?,
            minifig_detach_animation: row.get(4)?,
            mount_attach_animation: row.get(5)?,
            mount_detach_animation: row.get(6)?,
